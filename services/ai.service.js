...
async function generateBrandKit(inputs) {
  return chat([{ role: 'user', content: `Create a complete Brand Kit for:
...
Be very specific with HEX codes, font names, and visual descriptions.` }],
    'You are a world-class brand identity designer and strategist. Create detailed, professional brand systems.');
}

async function generateBusinessName(inputs) {
  return chat([{ role: 'user', content: `Generate business names for:
Industry: ${inputs.industry}
...
## Naming Tips
(3 specific tips for this industry)` }],
    'You are a professional naming consultant and brand strategist. Generate creative, memorable, market-ready business names.');
}

async function generateSlogan(inputs) {
  return chat([{ role: 'user', content: `Create slogans/taglines for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
(How to A/B test these with real customers)` }],
    'You are an award-winning copywriter specializing in brand slogans and taglines.');
}

// ── PLANNING & STRATEGY ─────────────────────────────────────────────────────
async function generateBusinessPlan(inputs) {
  return chat([{ role: 'user', content: `Generate a comprehensive, bank-ready business plan:
...
Make every section detailed, specific, and investor-ready.` }],
    'You are a senior business plan writer with 20 years experience creating plans for banks, investors, and accelerators.');
}

async function generateCompetitorMatrix(inputs) {
  return chat([{ role: 'user', content: `Create a comprehensive Competitor Analysis Matrix for:
...
## Battle Plan
(5 specific strategies to beat each competitor)` }],
    'You are a competitive intelligence analyst. Provide deep, actionable competitor analysis.');
}

async function generatePricingCalculator(inputs) {
  return chat([{ role: 'user', content: `Build a complete Pricing & Monetization Strategy for:
Business: ${inputs.businessName}
...
(When and how to raise prices)` }],
    'You are a pricing strategist and financial consultant. Create precise, profitable pricing strategies.');
}

async function generateLaunchRoadmap(inputs) {
  return chat([{ role: 'user', content: `Create a detailed 30-Day Launch Roadmap for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
Be extremely specific — real tasks, real tools, real actions every single day.` }],
    'You are a startup launch specialist. Create actionable, day-by-day launch plans.');
}

// ── LEGAL & ADMIN ───────────────────────────────────────────────────────────
async function generateContract(inputs) {
  return chat([{ role: 'user', content: `Generate a professional ${inputs.contractType || 'Service Agreement'} for:
...
Note: This is a template — recommend legal review before signing.` }],
    'You are a business attorney specializing in commercial contracts and agreements.');
}

async function generateBudgetEstimator(inputs) {
  return chat([{ role: 'user', content: `Create a detailed 6-Month Startup Budget for:
...
(When to expect profitability)` }],
    'You are a startup CFO and financial planning expert.');
}

async function generatePitchDeck(inputs) {
  return chat([{ role: 'user', content: `Create a complete Investor Pitch Deck script for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
For each slide: exact headline, 3-5 bullet points, speaker notes, and visual description.` }],
    'You are a Silicon Valley pitch coach who has helped companies raise over $500M.');
}

// ── MARKETING & ADVERTISING ─────────────────────────────────────────────────
async function generateAdCopy(inputs) {
  return chat([{ role: 'user', content: `Write platform-specific ad copy for:
Business: ${inputs.businessName}
Product/Service: ${inputs.product}
...
(Exact targeting settings for each platform)` }],
    'You are a performance marketing expert who manages millions in ad spend. Write ads that convert.');
}

async function generateSeoKeywords(inputs) {
  return chat([{ role: 'user', content: `Create a complete SEO Strategy & Keyword Map for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
(Month-by-month actions)` }],
    'You are a senior SEO strategist. Create data-driven, actionable SEO strategies.');
}

async function generateContentCalendar(inputs) {
  return chat([{ role: 'user', content: `Generate a 30-Day Social Media Content Calendar for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
(What metrics to track and how)` }],
    'You are a social media strategist and content creator. Generate engaging, platform-optimized content.');
}

async function generateMarketStudy(inputs) {
  return chat([{ role: 'user', content: `Conduct a comprehensive Market Study & Audience Analysis for:
...
(Ranked by conversion potential)` }],
    'You are a senior market research analyst with expertise in consumer behavior.');
}

// ── SALES & COMMUNICATION ───────────────────────────────────────────────────
async function generateColdEmail(inputs) {
  return chat([{ role: 'user', content: `Write a complete Cold Email Outreach Sequence for:
...
(Top 7 objections with exact responses)` }],
    'You are a top B2B sales expert specializing in cold outreach and pipeline generation.');
}

async function generateWebsiteCopy(inputs) {
  return chat([{ role: 'user', content: `Write high-converting website copy for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
...
## META DESCRIPTIONS
(For each page — SEO optimized)` }],
    'You are a world-class direct-response copywriter. Write copy that converts visitors into customers.');
}

async function generateSalesScript(inputs) {
  return chat([{ role: 'user', content: `Create a complete Sales Script for:
Business: ${inputs.businessName}
Product/Service: ${inputs.product}
...
## Deal Lost Recovery Script` }],
    'You are a top-performing sales trainer. Create scripts that close deals consistently.');
}

// ── MARKET RESEARCH (original) ──────────────────────────────────────────────
async function generateMarketResearch(inputs) {
  return chat([{ role: 'user', content: `Conduct comprehensive market research for:
Industry/Niche: ${inputs.niche}
Region: ${inputs.region || 'Global'}
...
9. Market Entry Recommendations` }],
    'You are a senior market research analyst. Provide deep, data-driven market insights.');
}

async function generateMarketingStrategy(inputs) {
  return chat([{ role: 'user', content: `Create a comprehensive marketing strategy for:
Business: ${inputs.businessName}
Industry: ${inputs.industry}
Target Audience: ${inputs.targetAudience}
Budget: ${inputs.budget || 'Not specified'}
...
Provide complete strategy with content plan, SEO, social media, paid ads, KPIs, and 90-day roadmap.` }],
